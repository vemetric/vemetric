...
const variantMap: Record<PaginationVariant, ButtonVariantMap> = {
  outline: { default: 'ghost', ellipsis: 'plain', current: 'surface' },
...
export const PaginationItem = React.forwardRef<HTMLButtonElement, ChakraPagination.ItemProps>(
  function PaginationItem(props, ref) {
    ...
    if (getHref) {
      return (
        <LinkButton href={getHref(props.value)} zIndex={current ? 1 : undefined} variant={variant} size={size}>
          {props.value}
        </LinkButton>
      );
    }
    ...
    return (
      <ChakraPagination.Item ref={ref} {...props} asChild>
        <Button zIndex={current ? 1 : undefined} variant={variant} size={size}>
          {props.value}
        </Button>
