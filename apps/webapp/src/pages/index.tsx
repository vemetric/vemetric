...
function Page() {
  const { data: projects, isLoading: isProjectsLoading } = trpc.projects.overview.useQuery();

  return (
    <BaseLayout>
      <VStack flex="1" gap="8" py="10">
        <Heading asChild size="2xl" textAlign="center">
        ...
        <motion.div
          ...
          style={{ width: '100%' }}
        >
          <SimpleGrid columns={{ base: 2, md: 3, lg: 4 }} gap={8} p={4} w="100%">
            {isProjectsLoading ? (
              <>
                <For each={[1, 2, 3]}>
                  {(_, index) => (
